...
const { extractAndAnonymize } = require('./pdf-anonymize');

// ---------- .env loader (dev local uniquement — no-op sur Vercel) ----------
function loadEnv() {
...
async function anonymizePdf(pdfPathOrBuffer) {
  const buffer = Buffer.isBuffer(pdfPathOrBuffer)
    ? pdfPathOrBuffer
    : fs.readFileSync(pdfPathOrBuffer);
  return extractAndAnonymize(buffer);
}
...
// ---------- API publique ----------
async function extractFromPdf(pdfPathOrBuffer) {
  const anonymized = await anonymizePdf(pdfPathOrBuffer);
