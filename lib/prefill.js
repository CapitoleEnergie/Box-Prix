...
const PREFILL_FIELDS = [
  ...
  'Offre__r.Compteur__c', 'Offre__r.Name', 'LastModifiedDate',
];
...
async function byAccount(accountId) {
  ...
  let where = `Offre__r.Compteur__r.Compte__c = '${id}'`;
  const q = `SELECT ${PREFILL_FIELDS.join(', ')} FROM LigneOffre__c WHERE ${where} AND TypeLigne__c IN ('Actuelle','Actuellement') ORDER BY LastModifiedDate DESC LIMIT 200`;
  let rows = await sf.query(q);
  if (!rows.length) {
    where = `Offre__r.Compteur__r.Compte_Siege__c = '${id}'`;
    const q2 = `SELECT ${PREFILL_FIELDS.join(', ')} FROM LigneOffre__c WHERE ${where} AND TypeLigne__c IN ('Actuelle','Actuellement') ORDER BY LastModifiedDate DESC LIMIT 200`;
    rows = await sf.query(q2);
  }
