...
function simulateCompteur(compteur, actuelLine, estimeLine, ref, overrides = {}) {
  ...
  // Prix moyen pondéré NON margé (€/MWh) = budget fourniture énergie hors marge / volume.
  // Réplique le champ LigneOffre__c.Prix_Moyen_Pondere_Non_Marge__c.
  // Colonne actuelle : pas de marge saisie -> calculTarif est déjà hors marge.
  // Colonne estimée : on retranche la marge globale appliquée pour rester hors marge.
  const margeA = v(actuelLine && actuelLine.margeGlobal);
  const margeE = v(estimeLine && estimeLine.margeGlobal);
  const prixMoyenNonMargeActuel = vol === 0 ? 0 : round2(actuel.calculTarif / vol - margeA);
  const prixMoyenNonMargeEstime = vol === 0 ? 0 : round2(estime.calculTarif / vol - margeE);
  ...
  return {
    ...
    prixMoyenActuel,
    prixMoyenEstime,
    prixMoyenNonMargeActuel,
    prixMoyenNonMargeEstime,
    flags: {
