...
function query(soql, { tooling = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    execFile(`"${SF_PATH}"`, args, { maxBuffer: 64 * 1024 * 1024, windowsHide: true, shell: true }, (err, stdout, stderr) => {
      fs.unlink(file, () => {});
      let parsed = null;
      try { parsed = JSON.parse(stdout); } catch (_) { /* ignore */ }
      if (parsed && parsed.status === 0 && parsed.result) return resolve(parsed.result.records || []);
      const msg = (parsed && parsed.message) || stderr || (err && err.message) || 'Erreur SOQL inconnue';
